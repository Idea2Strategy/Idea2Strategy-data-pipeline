...
--
-- `market_data` is D's schema, so this row cannot travel in the backend migration that seeds
-- `strategy.element_definitions` — `DatabaseAccessPolicy.verifyMigrationOwnership` refuses a
-- backend-owned migration that writes `market_data`, which is exactly the boundary that kept the
-- catalog unseeded from being fixed in one place.
--
